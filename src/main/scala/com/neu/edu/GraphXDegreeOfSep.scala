...
object GraphXDegreeOfSep {
  ...
  def main(args: Array[String]) {
    ...
    val sc = new SparkContext("local[*]", "GraphXDegreeOfSep")
    ...
    val bfs = initialGraph.pregel(Double.PositiveInfinity, 10)( // initial msg with inifinity an diteration 10

      (id, attr, msg) => math.min(attr, msg), //preserve shortest distance as we iterate through


      triplet => {
        if (triplet.srcAttr != Double.PositiveInfinity) {
          Iterator((triplet.dstId, triplet.srcAttr + 1)) // send msg to neighbouring node +1
        } else {
          Iterator.empty
        }
      },
      ...
      (a, b) => math.min(a, b)).cache() // preserved minimum at reducer
